...
export const getSubscriptionById = async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const SubscriptionTransactionDetails = await razorpay.subscriptions.fetch(id);
    console.log(SubscriptionTransactionDetails);

    res.status(200).json({ success: true, subscription: SubscriptionTransactionDetails });
  } catch (error) {
